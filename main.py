...
def entire_text_to_int(text, vocab):
	...
	i = 0
	while (i < len(text)):
		int_array = np.append(int_array, letter_to_int((text[i]), vocab))
		i+=1
		print(i)
	return int_array
...
def entire_int_to_text(num_array, vocab):
	...
	while (i < len(num_array)):
		curr_letter = vocab[int(num_array[i])]
		text_array = np.append(text_array, curr_letter)
		i+=1
		print(i)
	return text_array
...
print(generate_text(model, start_string=u"Yeah right on"))
